<?php
    ...
    class TableRegistry
    {
        ...
        public function getAllTableStatus(): array
        {
            ...
            foreach ($this->tables as $k => $tableObject)
            {
                $isTableCreated = $tableObject->isTableCreated();

                $data[$tableObject->getName()] = [
                    'is_created' => (int)$isTableCreated,
                    'count'      => $isTableCreated ? (int)$tableObject->getCount() : 0,
                ];
            }
...
<?php
    ...
    {
        protected string $comment = '__COMMENT__';
